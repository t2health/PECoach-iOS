...
+(MKInfoPanel *)showPanelInView:(UIView *)view type:(MKInfoPanelType)type title:(NSString *)title subtitle:(NSString *)subtitle hideAfter:(NSTimeInterval)interval {    
    ...
    if(subtitle) {        
        ...
        // If we have a title and a subtitle, combine them for what is said for Accessibility (VoiceOver)
        [panel.titleLabel.text setAccessibilityLabel:@" "];
        [panel.titleLabel.text setAccessibilityHint:[title stringByAppendingString:subtitle]];
        [panel.detailLabel.text setAccessibilityLabel:@" "];
        [panel.detailLabel.text setAccessibilityHint:[title stringByAppendingString:subtitle]];
...
// TBD - This should utilize the above method!  Don't reinvent the wheel...
// 06/06/12 Add this for panels that include special characters (newline, tabs, etc).  Size calculations get messed up for these
+ (MKInfoPanel *)showPanelInView:(UIView*)view type:(MKInfoPanelType)type title:(NSString *)title subtitle:(NSString *)subtitle hideAfter:(NSTimeInterval)interval width:(CGFloat)width height:(CGFloat)height {
    MKInfoPanel *panel = [MKInfoPanel infoPanel];
    CGFloat panelHeight = 50;   // panel height when no subtitle set
    ...
    if(subtitle) {
        ...
        // 06/05/12 Sizing is causing a problem with embedded tabs...only do this if we did not specify a fixed size
        if (height == 0.0) {
            [panel.detailLabel sizeToFit]; 
        } else {
            // Set the customized size of the detail label 
            panel.detailLabel.frame = CGRectMake(panel.detailLabel.frame.origin.x, panel.detailLabel.frame.origin.y, width, height);
        }
